// fs para gestión de documentos. Hará falta para sobrescribir el archivo con el historial
const fs = require("fs");
...
class Busquedas {
  ...
  async pokemon_specs( pokemon ) {
    try {
      ...
      // Se usa await para esperar a que el valor 'axios_input' esté definido y no se use dicha variable mientras se hace el GET
      const axios_input = await instance.get();
      
      // Los metadatos de la API de axios tiene distintos datos (código de estado, headers, url pedida, milisegundos de la query...)
      // Solo nos interesan los metadatos (la 'data' de la instancia)
      let pokemon_data = axios_input.data;

      // Los pokemon pueden tener más de un tipo. 
      // Si permitimos que, en ciertos casos, nuestro programa lea dos tipos del pokemon cuando solo existe un tipo, todo el resultado (nombre, tipo, estadisticas, peso, altura)
      // se pierde, ya que el mensaje TypeError se activa (lee algo que es undefined) y el catch (error) se activa
      // Si el segundo tipo del pokemon no existe en ( pokemon_data ), queremos que no devuelva nada.
      let pokemon_type = "";
